<template>
  <div class="mx-auto md:mx-auto lg:mx-4 lg:w-96 xl:mr-16 my-auto">
    ...
    <div
      v-if="error"
      class="text-red-900 h-12 flex items-center w-11/12 text-center border-2 border-red-600 mb-2 bg-red-100"
    >
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <button
        class="text-white bg-black p-2 w-52 sm:w-60 md:w-96 lg:w-11/12 rounded-md mt-6"
      >
        Login
      </button>
      <br />
      <button class="text-center mt-12">
        ...
        <router-link to="/signupAs"
          ><span class="font-bold">Signup</span></router-link
        >
      </button>
    </form>
  </div>
    ...
    async handleSubmit() {
      try {
        ...
        localStorage.setItem(
          "name",
          unwrapped.user.first_name + " " + unwrapped.user.last_name
        );
